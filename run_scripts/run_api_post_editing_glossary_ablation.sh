#!/bin/bash
# Ablation experiment for top_n_per_word_glossary parameter
# Testing values: 3, 10, 15, and 20 (already tested 5)
# Only running on webp variant

# Set working directory to project root
cd /data/projects/punim0478/setiawand/bible-nmt

# Load environment variables from .env file
if [ -f .env ]; then
    echo "Loading environment variables from .env file..."
    export $(cat .env | grep -v '^#' | xargs)
else
    echo "Warning: .env file not found at $(pwd)/.env"
fi

# Change to post-editing directory for imports to work correctly
cd src/post-editing

BASE_OUTPUT="/data/projects/punim0478/setiawand/bible-nmt/results"
GLOSSARY_PATH="/data/projects/punim0478/setiawand/bible-nmt/lexical-resource/eng-dhao/dhao-english-dict-full.csv"
SRC="eng"
TGT="nfa"
SRC_LANG_NAME="English"
TGT_LANG_NAME="Dhao"


# WEBP variant experiments (ACTIVE)
CSV_PATH="/data/projects/punim0478/setiawand/bible-nmt/ebible-corpus/dhao-eng/engwebp/aligned-eng-engwebp-ot.csv"

# Loop through different top_n_per_word_glossary values for webp
for n in 50 70 100; do
    echo "Running webp experiment with top_n_per_word_glossary=${n}..."
    
    python run_post_editing.py \
        --model_type "gemini" \
        --model_name "gemini-2.5-flash" \
        --csv_path ${CSV_PATH} \
        --src "eng" \
        --tgt ${TGT} \
        --src_lang_name ${SRC_LANG_NAME} \
        --tgt_lang_name ${TGT_LANG_NAME} \
        --output_dir "${BASE_OUTPUT}/gemini-2.5-flash-webp-dhao/glossary_n${n}" \
        --prompt "dhao_post_editing" \
        --few_shot_mode "glossary" \
        --glossary_mode "word_fuzzy" \
        --glossary_path ${GLOSSARY_PATH} \
        --batch_size 100 \
        --delay_between_batches 5.0 \
        --top_n_per_word_glossary ${n} \
        --max_samples 500 \
        --debug
    
    echo "Completed webp experiment with n=${n}"
    
    # Add delay between experiments except for the last one
    if [ ${n} != 30 ]; then
        echo "Sleeping for 2 minutes to avoid API rate limiting..."
        sleep 120
    fi
done

echo "All ablation experiments completed!"
echo "Results saved in:"
echo "  - ${BASE_OUTPUT}/gemini-2.5-flash-webp-dhao/glossary_n3"
echo "  - ${BASE_OUTPUT}/gemini-2.5-flash-webp-dhao/glossary_n10"
echo "  - ${BASE_OUTPUT}/gemini-2.5-flash-webp-dhao/glossary_n15"
echo "  - ${BASE_OUTPUT}/gemini-2.5-flash-webp-dhao/glossary_n20"
echo "  - ${BASE_OUTPUT}/gemini-2.5-flash-webp-dhao/glossary_n25"
echo "  - ${BASE_OUTPUT}/gemini-2.5-flash-webp-dhao/glossary_n30"
