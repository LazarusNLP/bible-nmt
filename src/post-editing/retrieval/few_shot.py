...
class FewShotSelector:
    ...
    def __init__(
        ...
        if vectorizer_type not in supported_types:
            raise ValueError(
                f"Unsupported vectorizer type: {vectorizer_type}. "
        ...
        if vectorizer_type == "full":
            ...
            self.retriever = SBERTRetriever(
                corpus_id=corpus_id, model_name="BAAI/bge-large-en-v1.5"
            )
        elif vectorizer_type == "random":
            # Use random retriever as baseline
            self.retriever = RandomRetriever()
    ...
    def get_examples_for_rows(
        self, rows: List[Any], corpus: List[Tuple], k: int
    ) -> List[List[Tuple]]:
        """
        ...
        if self.vectorizer_type == "full":
            print(
                f"Using FULL mode: providing all {len(corpus)} corpus examples for each of {len(rows)} rows..."
            )
        ...
        if hasattr(self.retriever, "get_similar_examples_batch"):
            print(
                f"🚀 Using OPTIMIZED BATCH processing for {len(rows)} rows with {self.vectorizer_type} vectorizer..."
            )
            ...
            if self.vectorizer_type == "word_parallel" and k == -1:
                print(
                    f"Using WORD_PARALLEL mode (individual processing): providing all word-matched examples for each of {len(rows)} rows..."
                )
            else:
                print(
                    f"Selecting few-shot examples for {len(rows)} rows using {self.vectorizer_type} (individual processing)..."
                )
