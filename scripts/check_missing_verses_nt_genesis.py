#!/usr/bin/env python3
"""
Missing Verse Checker - NT + Genesis Focus

This script checks for missing verses between English Bible versions and the Dhao (nfa) Bible,
focusing ONLY on New Testament + Genesis books (the books that exist in the Dhao Bible).
It identifies cases where a verse exists in one version but not the other.
...
# Books that exist in the Dhao Bible: Genesis + New Testament
DHAO_BOOKS = ["GEN"] + NT_BOOKS
...
def is_dhao_book(verse_ref: str) -> bool:
    """Check if a verse reference is from a book that exists in the Dhao Bible."""
    if not verse_ref:
        return False
    book = verse_ref.split()[0]
    return book in DHAO_BOOKS


def get_verse_content_status(text_lines: List[str], vref_lines: List[str]) -> Dict[str, str]:
    """
    Get the content status for each verse reference, filtering to only Dhao books.
    ...
    for idx, (text_line, vref_line) in enumerate(zip(text_lines, vref_lines)):
        ...
        # Skip verses not in Dhao books
        if not is_dhao_book(verse_ref):
            processed.add(idx)
            continue
        ...
        if idx in range_map:
            ...
            for i in range(start_idx + 1, end_idx + 1):
                if i < len(vref_lines):
                    ...
                    if is_dhao_book(range_verse_ref):
...
def compare_versions(eng_status: Dict[str, str], dhao_status: Dict[str, str], 
    ...
    # Get all verse references that are in both dictionaries (should be same set since filtered)
...
def analyze_bible_versions(english_corpus_dir: str, dhao_corpus_path: str, vref_path: str):
    """
    Analyze all English Bible versions against Dhao version for missing verses in NT + Genesis only.
    """
    print("=== Missing Verse Analysis - NT + Genesis Focus ===")
    ...
    print(f"Analyzing books: {DHAO_BOOKS}")
    print()
    ...
    # Get Dhao verse status (filtered to Dhao books only)
    print("Analyzing Dhao verse content (NT + Genesis only)...")
    ...
    print(f"Dhao NT+Genesis status: {dhao_content_count} content, {dhao_empty_count} empty, {dhao_range_count} range")
    print(f"Total NT+Genesis verses: {len(dhao_status)}")
    print()
    ...
    for eng_file in eng_files:
        ...
        # Get English verse status (filtered to Dhao books only)
        ...
        if total_issues > 0:
            print(f"  Found {total_issues} mismatches in NT+Genesis:")
            ...
            print(f"  ✅ Perfect alignment! No mismatches in NT+Genesis")
        
        all_results[eng_version] = mismatches
        print()
    
    # Generate detailed report
    print("=== DETAILED MISMATCH REPORT (NT + Genesis) ===")
    print()
    
    # Create output directory
    output_dir = Path("missing_verses_nt_genesis_analysis")
    output_dir.mkdir(exist_ok=True)
    
    # Save detailed CSV report
    csv_file = output_dir / "missing_verses_nt_genesis_detailed.csv"
    ...
    # Summary report
    summary_file = output_dir / "missing_verses_nt_genesis_summary.csv"
    ...
    print("=== SUMMARY STATISTICS (NT + Genesis) ===")
    print(f"Total English versions analyzed: {len(eng_files)}")
    print(f"Total NT+Genesis verses checked: {len(dhao_status)}")
    print(f"Dhao verses with content: {dhao_content_count}")
    print()
    
    # Find perfect alignments (no mismatches)
    perfect_versions = []
    for eng_version, mismatches in all_results.items():
        total_issues = sum(len(issues) for issues in mismatches.values())
        if total_issues == 0:
            perfect_versions.append(eng_version)
    
    if perfect_versions:
        print(f"🎉 PERFECT ALIGNMENTS (no missing verses): {len(perfect_versions)} versions")
        for version in sorted(perfect_versions):
            print(f"  ✅ {version}")
        print()
    ...
    print("Best English versions for Dhao alignment (fewest mismatches):")
    for version, count in version_totals[:10]:
        status = "✅ Perfect!" if count == 0 else f"{count} mismatches"
        print(f"  {version}: {status}")
    print()
    
    if version_totals[-5:][0][1] > 0:
        print("Versions with most mismatches:")
        for version, count in version_totals[-5:]:
            print(f"  {version}: {count} mismatches")
        print()
    
    # Show some example verses with issues (if any)
    if total_mismatches:
...
def main():
    parser = argparse.ArgumentParser(
        description="Check for missing verses between English Bible versions and Dhao Bible (NT + Genesis only)"
    )
    parser.add_argument(
        "--english_corpus_dir",
        type=str,
