...
class SymmetricAlignmentScorer:
    def __init__(self, english_corpus_dir: str, dhao_corpus_path: str, vref_path: str, 
    ...
    def normalize_text_file(self, input_path: Path, output_path: Path) -> bool:
        ...
        try:
            cmd = [
                sys.executable, "scripts/normalize_text.py",
                str(input_path),
                "-o", str(output_path),
    ...
    def create_aligned_corpus(self, source_path: Path, target_path: Path, 
                            output_prefix: str) -> Tuple[Path, bool]:
        """Create aligned corpus using create_aligned_nt_ot.py script."""
        try:
            output_file = self.aligned_dir / f"{output_prefix}.csv"
            
            cmd = [
                sys.executable, "scripts/create_aligned_nt_ot.py",
                "--source_corpus_path", str(source_path),
                "--target_corpus_path", str(target_path),
    ...
    def prepare_giza_input(self, aligned_csv: Path, output_prefix: str) -> Tuple[Path, Path, bool]:
        ...
        try:
            ...
            with open(aligned_csv, 'r', encoding='utf-8') as csvfile:
                ...
                with open(source_file, 'w', encoding='utf-8') as src_f, \
                     open(target_file, 'w', encoding='utf-8') as tgt_f:
    ...
    def run_giza_alignment(self, source_file: Path, target_file: Path, 
                          output_prefix: str, direction: str = "forward") -> Tuple[float, bool]:
        ...
        try:
            ...
            # Convert to GIZA format using freshly compiled plain2snt
            plain2snt_path = "/data/projects/punim0478/setiawand/giza-pp-new/GIZA++-v2/plain2snt.out"
            plain2snt_cmd = [
                plain2snt_path,
                "source.txt",
                "target.txt"
            ...
            result = subprocess.run(plain2snt_cmd, cwd=giza_output_dir, 
                                  capture_output=True, text=True)
            
            if result.returncode != 0:
                logger.error(f"plain2snt failed for {output_prefix} {direction}: {result.stderr}")
                return 0.0, False
            ...
            if not all(f.exists() for f in [src_vcb, tgt_vcb, snt_file]):
                logger.error(f"plain2snt did not create expected files for {output_prefix} {direction}")
                logger.debug(f"Expected files: {src_vcb}, {tgt_vcb}, {snt_file}")
                return 0.0, False
            
            # Create cooccurrence file using freshly compiled version
            snt2cooc_path = "/data/projects/punim0478/setiawand/giza-pp-new/GIZA++-v2/snt2cooc.out"
            snt2cooc_cmd = [
                snt2cooc_path,
                str(cooc_file),
                str(src_vcb),
            ...
            logger.debug(f"Running snt2cooc: {' '.join(snt2cooc_cmd)}")
            result = subprocess.run(snt2cooc_cmd, cwd=giza_output_dir,
                                  capture_output=True, text=True)
            
            if result.returncode != 0:
                logger.error(f"snt2cooc failed for {output_prefix} {direction}: {result.stderr}")
                return 0.0, False
            
            # Create symbolic links for the expected filename format
            src_vcb_file = giza_output_dir / "source.vcbfile"
            tgt_vcb_file = giza_output_dir / "target.vcbfile"
            
            # Create symbolic links if they don't exist
            if not src_vcb_file.exists():
                src_vcb_file.symlink_to(src_vcb.name)
            if not tgt_vcb_file.exists():
                tgt_vcb_file.symlink_to(tgt_vcb.name)
            
            # Use freshly compiled GIZA++ binaries
            giza_path = "/data/projects/punim0478/setiawand/giza-pp-new/GIZA++-v2/GIZA++"
            snt2cooc_path = "/data/projects/punim0478/setiawand/giza-pp-new/GIZA++-v2/snt2cooc.out"
            plain2snt_path = "/data/projects/punim0478/setiawand/giza-pp-new/GIZA++-v2/plain2snt.out"
            ...
            giza_cmd = [
                giza_path,
                "-s", str(src_vcb),  # Use .vcb directly, not .vcbfile
                "-t", str(tgt_vcb), 
                "-c", str(snt_file),
                "-CoocurrenceFile", str(cooc_file),
                "-o", f"{output_prefix}_{direction}",
                "-outputpath", str(giza_output_dir),
                # Minimal parameters to avoid segfault
                "-m1", "1",  # IBM Model 1 iterations (minimal)
                "-m2", "0",  # Skip IBM Model 2 
                "-mh", "0",  # Skip HMM
                "-m3", "0",  # Skip IBM Model 3
                "-m4", "0",  # Skip IBM Model 4
                "-nodumps"   # Don't create dump files
            ]
            
            result = subprocess.run(giza_cmd, cwd=giza_output_dir,
                                  capture_output=True, text=True, timeout=300)
            
            if result.returncode != 0:
                logger.warning(f"GIZA++ failed for {output_prefix} {direction} (exit code {result.returncode})")
                logger.warning(f"GIZA++ stderr: {result.stderr[:500]}...")  # Truncate long error messages
                
                # Fallback to simple word overlap scoring
                logger.info(f"Falling back to word overlap scoring for {output_prefix} {direction}")
            ...
            logger.debug(f"GIZA++ {direction} alignment completed for {output_prefix}, log-likelihood: {log_likelihood}")
    ...
    def calculate_word_overlap_score(self, src_file: Path, tgt_file: Path) -> float:
        ...
        try:
            ...
            if len(src_lines) != len(tgt_lines):
                logger.warning(f"Mismatched line counts: {len(src_lines)} vs {len(tgt_lines)}")
            ...
            logger.debug(f"Word overlap score: {score:.3f} (avg overlap: {avg_overlap:.3f}, pairs: {total_pairs})")
            return score
    ...
    def process_single_version(self, english_file: Path) -> Dict[str, any]:
        ...
        try:
            ...
            source_file, target_file, prep_success = self.prepare_giza_input(aligned_csv, version_name)
            ...
            if forward_success and reverse_success:
                ...
                logger.info(f"Successfully processed {version_name}: forward={forward_score:.4f}, reverse={reverse_score:.4f}, avg={average_score:.4f}")
            else:
    ...
    def run_scoring(self) -> List[Dict[str, any]]:
        ...
        logger.info(f"Starting symmetric alignment scoring for {len(english_files)} English versions")
    ...
    def save_results(self, results: List[Dict[str, any]], output_file: Path):
        ...
        try:
            with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
                fieldnames = ['english_version', 'target_language', 'forward_score', 
                            'reverse_score', 'average_score']
                writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                
                writer.writeheader()
                for result in results:
                    if result['success']:
                        writer.writerow({k: v for k, v in result.items() if k != 'success'})
            
            logger.info(f"Results saved to: {output_file}")
            
            # Print summary
            successful_results = [r for r in results if r['success']]
            logger.info(f"Successfully processed {len(successful_results)}/{len(results)} versions")
            ...
            if successful_results:
                ...
                for i, result in enumerate(successful_results[:5], 1):
                    logger.info(f"{i}. {result['english_version']}: avg={result['average_score']:.4f}")
